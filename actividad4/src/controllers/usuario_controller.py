...
def listar_usuarios():
    ...
    return render_template('forms/usuarios/listar_todo.html', usuarios=usuarios, computadores=computadores)
...
def guardar_usuario():
    ...
    nuevo_usuario = Usuario(username=username, password=password, nombre=nombre, email=email)
...
def actualizar_usuario():
    ...
    if usuario_existente:
        ...
        flash(f"Error: Usuario con ID {usuario_id} no encontrado para actualizar.", 'danger')
        return redirect(url_for('usuario_bp.listar_usuarios'))
...
def buscar_usuario():
    ...
    return render_template('forms/usuarios/buscar_resultado.html', 
...
def procesar_login():
    ...
    if admin_login == "true" and username_or_email == "admin" and password == "admin":
...
def mostrar_detalles_usuario():
   ...
   if usuario_id_logueado == -1 and usuario_username_logueado == "admin":
		usuario_logueado = Usuario(username="admin", password="", nombre="Administrador", email="")
        usuario_logueado.id = -1
    else:
        usuario_logueado = usuario_service.obtener_usuario_por_id(usuario_id_logueado)
    ...
    print(f"DEBUG (Flask): usuarioLogueado en sesión: {usuario_logueado.username if usuario_logueado else 'None'}")

    return render_template('forms/usuarios/listar.html', usuarioLogueado=usuario_logueado)
