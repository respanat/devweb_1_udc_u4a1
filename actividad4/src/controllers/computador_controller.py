...
def listar_computadores():
    ...
    return render_template('forms/computadores/listar_todo.html', computadores=computadores)


@computador_bp.route('/agregar', methods=['GET'])
def mostrar_formulario_agregar():
    computador = Computador(marca="", categoria="", marca_cpu="", velocidad_cpu=0.0,
                            tecnologia_ram="", capacidad_ram=0, tecnologia_disco="",
                            capacidad_disco=0, num_puertos_usb=0, num_puertos_hdmi=0,
                            marca_monitor="", pulgadas=0.0, precio=0.0)
    
    usuario_service: UsuarioService = current_app.usuario_service
    usuarios = usuario_service.obtener_todos_los_usuarios()
    
    return render_template('forms/computadores/agregar.html', computador=computador, usuarios=usuarios)
...
def mostrar_formulario_editar():
    ...
    return render_template('forms/computadores/editar.html', computador=computador, usuarios=usuarios)
...
def actualizar_computador():
    ...
    computador_existente = computador_service.obtener_computador_por_id(computador_id)
    ...
    computador_existente.num_puertos_hdmi = int(request.form['num_puertos_hdmi'])
    ...
    if usuario_id:
        ...
        if usuario_obj:
            ...
            flash(f"Advertencia: Usuario con ID {usuario_id} no encontrado. Relación no establecida.", 'warning')
    else:
...
def buscar_computador():
    ...
    return render_template('forms/computadores/buscar_resultado.html', 
